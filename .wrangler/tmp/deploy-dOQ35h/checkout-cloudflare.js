...
function corsJson(body, status = 200, corsHeaders) {
...
async function handleCheckout(body, stripe, siteUrl, config2) {
  ...
  const billingAddress = config2?.billingAddress ?? "required";
  const shippingCountries = config2?.shippingCountries ?? ["US", "GB"];
...
// functions/checkout-cloudflare.ts
var checkout_cloudflare_default = {
  async fetch(request, env2) {
    ...
    if (url.pathname === "/checkout" && request.method === "POST") {
      ...
      try {
        const stripe = new stripe_esm_worker_default(env2.STRIPE_SECRET_KEY);
        const siteUrl = (env2.SITE_URL || "").replace(/\/$/, "");
        const config2 = {
          billingAddress: env2.STRIPE_BILLING_ADDRESS || "required",
          shippingCountries: (env2.STRIPE_SHIPPING_COUNTRIES || "US,GB").split(",").map((c) => c.trim())
        };
        const result = await handleCheckout(body, stripe, siteUrl, config2);
...
export {
  checkout_cloudflare_default as default
};
//# sourceMappingURL=checkout-cloudflare.js.map
